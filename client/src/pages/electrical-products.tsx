...
export default function ElectricalProducts() {
  const { data: electricalproducts, isLoading, error } = useQuery<Product[]>({
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        {error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            ) : electricalproducts && electricalproducts.length > 0 ? (
              electricalproducts.map((electricalproduct) => (
                <ProductCard key={electricalproduct.id} product={electricalproduct} />
              ))
            ) : (
              <EmptyState />
            )}
